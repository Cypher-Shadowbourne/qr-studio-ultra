...
fn build_base_qr_svg(options: &QrOptions, size: u32) -> Result<String, String> {
    ...
    let ecl = match options.error_correction_level.as_deref() {
        Some("L") => ErrorCorrectionLevel::L,
        Some("M") => ErrorCorrectionLevel::M,
        Some("Q") => ErrorCorrectionLevel::Q,
    ...
    let mut builder = QRCodeStyling::builder()
        ...
        .size(size)
        .margin(12)
        .qr_options(QROptions::new().with_error_correction_level(ecl))
        .dots_options(dots)
...
fn generate_ultra_qr(options: QrOptions) -> Result<String, String> {
    ...
    let ecl = match options.error_correction_level.as_deref() {
        Some("L") => fast_qr::ECL::L,
        Some("M") => fast_qr::ECL::M,
        Some("Q") => fast_qr::ECL::Q,
        _ => fast_qr::ECL::H,
    };
